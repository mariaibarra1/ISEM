...
namespace CatalogosService.Controllers
{
    ...
    {
        private CaRolNegocio negocio = new CaRolNegocio();

        [HttpGet("[action]")]
        public IActionResult Consultar([FromQuery] CaRolModel model)
        {
            model.tipoOperacion = (int)TipoOperacion.consultar;
            return this.ProcesarConsulta(model);
        }

        private IActionResult ProcesarConsulta(CaRolModel model)
        {
            try
            {

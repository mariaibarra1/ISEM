...
namespace CatalogosService.Controllers
{
    ...
    public class CaTipoOficioController : Controller
    {
        private CaTipoOficioNegocio negocio = new CaTipoOficioNegocio();

        [HttpGet("[action]")]
        public IActionResult Consultar([FromQuery] CaTipoOficioModel model)
        {
            model.tipoOperacion = (int)TipoOperacion.consultar;
            return this.ProcesarConsulta(model);
        }

        private IActionResult ProcesarConsulta(CaTipoOficioModel model)
        {
            try
            {
